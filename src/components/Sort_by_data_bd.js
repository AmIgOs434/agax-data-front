...
import { DOSTUP_ROUTE, GLAV_ROUTE, MENU_ROUTE, SORT_BY_DATA_OP_BD_ROUTE, SORT_BY_RAZDEL, SORT_BY_RAZDEL1, USER_ROUTE, VLADELETS_ROUTE } from '../utils/consts';
import moment from 'moment';
function Sort_by_data_bd() {
  ...
  const get1 = async() => {
    ...
    const storedToken = localStorage.getItem('token');
      ...
      if(storedToken==null || storedToken==undefined){
        ...
        prihod.map( e =>{ 
              if(e['Приходный счет']==id && e['Расходный счет']!=id ){
                arr_3.push({data:e.DateCreate,kod:e.Код,prihod:parseFloat(e.Сумма),rashod:'',balans:'',info:`${e.Содержание} / ${e.Договор} / Дата операции: ${moment.utc(e.Дата).format('DD.MM.YYYY ')}`})
              }
              if(e['Расходный счет']==id && e['Приходный счет']!=id ){
                arr_3.push({data:e.DateCreate,kod:e.Код,prihod:'',rashod:parseFloat(e.Сумма) ,balans:'',info:`${e.Содержание} / ${e.Договор} / Дата операции: ${moment.utc(e.Дата).format('DD.MM.YYYY ')}`})
              }
 
              if(e['Расходный счет']==id && e['Приходный счет']==id ){
                arr_3.push({data:e.DateCreate,kod:e.Код,prihod:parseFloat(e.Сумма),rashod:parseFloat(e.Сумма),balans:'',info:`${e.Содержание} / ${e.Договор} / Дата операции: ${moment.utc(e.Дата).format('DD.MM.YYYY ')}`})
              }
        })
     arr_3.sort(function(a,b){
        ...
        arr_3.map(e=> e.data = moment.utc(e.data).format('DD.MM.YYYY HH:mm:ss'))
...
export default Sort_by_data_bd;
